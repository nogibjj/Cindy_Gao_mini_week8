...
fn test_extract() {
    ...
    assert!(
        ...
        "File was not found after extraction"
    );
}
...
fn test_update_query() {
    ...
    let mut stmt = conn
        .prepare("SELECT change FROM Murder2015 WHERE city = 'Chicago'")
...
fn test_delete_query() {
    ...
    let conn = Connection::open("Murder2015.db").expect("Failed to open database");
    let mut stmt = conn
        .prepare("SELECT COUNT(*) FROM Murder2015 WHERE city = 'Chicago'")
        .expect("Failed to prepare statement");
    let count: i32 = stmt
        .query_row([], |row| row.get(0))
        .expect("Failed to fetch row");
...
fn test_sorting_change() {
    setup_database().expect("Failed to set up the database");
    assert!(
        sorting_change().is_ok(),
        "Failed to sort and retrieve data from database"
    );
}
